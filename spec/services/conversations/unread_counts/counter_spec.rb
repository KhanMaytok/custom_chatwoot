...
RSpec.describe Conversations::UnreadCounts::Counter do
  ...
  it 'builds the assignment cache on demand for normal agents' do
  ...
  it 'uses a Redis lock while building the assignment cache on demand' do
    lock_key = "UNREAD_CONVERSATIONS::V1::ACCOUNT::#{account.id}::BUILD_LOCK::ASSIGNMENT"
  ...
  it 'waits instead of rebuilding when another process owns the assignment build lock' do
    lock_manager = instance_double(Redis::LockManager, with_lock: false)
    counter = described_class.new(account: account, user: agent)

    allow(Redis::LockManager).to receive(:new).and_return(lock_manager)
    allow(counter).to receive(:wait_for_cache_ready) { store.mark_assignment_ready!(account.id) }
  ...
  it 'counts only assigned unread conversations across inboxes visible to a normal agent' do
    create_unread_conversation(account: account, inbox: visible_inbox, labels: [label.title], assignee: agent, team: visible_team)
    create_unread_conversation(account: account, inbox: visible_inbox, labels: [label.title], assignee: create(:user, account: account), team: visible_team)
    create_unread_conversation(account: account, inbox: hidden_inbox, labels: [label.title], assignee: agent, team: visible_team)
  ...
  it 'does not return zero counts or labels hidden from the sidebar' do
    create_unread_conversation(account: account, inbox: visible_inbox, labels: [hidden_label.title], assignee: agent, team: visible_team)
